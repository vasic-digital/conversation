...
echo "=== conversation Stress Sustained-Load Challenge ==="
echo "  url=$HEALTH_URL dur=${DURATION}s rps=${RPS}"

if [[ -z "$HEALTH_URL" ]]; then
    echo "[1/6] SKIP: CONVERSATION_HEALTH_URL unset — SKIP-OK: #env-no-target"
    echo "=== conversation Stress Challenge: PASSED (SKIP-OK) ==="
    exit 0
fi
...
echo
echo "=== conversation Stress Challenge: PASSED ==="
